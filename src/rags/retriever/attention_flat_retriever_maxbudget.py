...
class AttentionFlatRetrieverMaxbudget(AbsRetriever):
    """
    ...
    def _retrieve(
        ...
        # !! maxbudget does not depend on the compression ratio
        ...
        # !! maxbudget
        # init_n_sentences_to_pick = min(
        #     init_n_sentences_to_pick,
        #     np.searchsorted(prefix_sum, self.retrieval_budget),
        # )
        # self.log.debug(
        #     "After capping with context_window_size (%d) n_sentences_to_pick: %d",
        #     self.context_window_size,
        #     init_n_sentences_to_pick,
        # )
        init_n_sentences_to_pick = np.searchsorted(prefix_sum, self.retrieval_budget)

        retrieved_chunklist = sorted_chunks[:init_n_sentences_to_pick]
        ...
        if self.retain_ctxt_order:
            ...
            for chunk in retrieved_chunklist:
                self.log.debug(
                    "Sorted! [chunk %15s rs:%.9f chunk_len: %d] %s ",
                    chunk.index,
                    chunk.reaction_score,
                    chunk.ntokens,
                    chunk.passage.replace("\n", "\\n"),
        ...
        return {
            ...
            "token_level_compression_rate": reaction_vec_len / retrieved_chunklist_len,
